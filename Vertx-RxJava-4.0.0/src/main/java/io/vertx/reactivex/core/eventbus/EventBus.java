...
public class EventBus {
	...
	@Trace
	public <T> Single<io.vertx.reactivex.core.eventbus.Message<T>> rxRequest(String address, Object message) { 
		Single<io.vertx.reactivex.core.eventbus.Message<T>> resultSingle = Weaver.callOriginal();
	...
	@Trace
	public <T> Single<io.vertx.reactivex.core.eventbus.Message<T>> rxRequest(String address, Object message, DeliveryOptions options) { 
		Single<io.vertx.reactivex.core.eventbus.Message<T>> resultSingle = Weaver.callOriginal();
		NRHolder holder = new NRHolder("EventBus", "rxSend", NewRelic.getAgent().getTransaction());
		return resultSingle.doOnSubscribe(new NRSubscribeConsumer(holder)).doOnSuccess(new NRCompletionConsumer<>(holder)).doOnError(new NRErrorConsumer(holder)).doFinally(new NRTerminateAction(holder));
	}
