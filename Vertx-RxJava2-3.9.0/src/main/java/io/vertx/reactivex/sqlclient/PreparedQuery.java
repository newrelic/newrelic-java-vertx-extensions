...
public abstract class PreparedQuery {
	...
	@Trace
	public void executeBatch(List<io.vertx.reactivex.sqlclient.Tuple> argsList, Handler<AsyncResult<io.vertx.reactivex.sqlclient.RowSet>> handler) {
		NRHandlerWrapper<RowSet> wrapper = new NRHandlerWrapper<io.vertx.reactivex.sqlclient.RowSet>(handler, NewRelic.getAgent().getTransaction().getToken());
		handler = wrapper;
		Weaver.callOriginal();
	}
